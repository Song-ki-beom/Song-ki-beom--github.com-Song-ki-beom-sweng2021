...
def processKey(board, key , observer):
	global nBlocks 

	state = board.accept(key)
	observer.writeScreen(board.getScreen())
...
class Model(threading.Thread, Observer,Publisher):
	...
	def run(self):
		...
		key = str(idxBlockType)
		state = board.accept(key)
		self.observer.writeScreen(board.getScreen())
		...
		while not isGameDone:
			...
			if key == 'q':
				...
				state = processKey(board, key, self.observer)
...
class View (threading.Thread, Observer):
	...
	def writeScreen(self,Screen):
		self.cv.acquire()
		printWindow(self.window, Screen)
		self.cv.notify()
		self.cv.release()
